...
const thDT = d => new Date(d).toLocaleString('th-TH', {
  day:'2-digit', month:'2-digit', year:'2-digit',
  hour:'2-digit', minute:'2-digit'
});

function stat(qty, sf) {
  if (qty === 0) return 'out';
  if (qty <= sf) return 'low';
  return 'ok';
}

// ── CSS-in-JS styles ────────────────────────────────────
const css = {
  wrap: { maxWidth:1100, margin:'0 auto', padding:'0 12px 60px', fontFamily:"Arial,'Noto Sans Thai',sans-serif", fontSize:13, color:'#1a1a1a' },
  hdr:  { background:'#1B3A2D', color:'#EFF7F3', padding:'12px 16px', borderRadius:10, marginBottom:12, display:'flex', justifyContent:'space-between', alignItems:'center' },
  ...
  kgrid:{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(155px,1fr))', gap:10, marginBottom:12 },
  ...
  th:   { background:'#1B3A2D', color:'#EFF7F3', padding:'7px 8px', textAlign:'left', fontWeight:500, whiteSpace:'nowrap' },
  tow:  { overflowX:'auto' },
};
const btn = (v='dk') => {
  const m = { dk:['#1B3A2D','#EFF7F3'], gr:['#1E8449','#fff'], rd:['#C0392B','#fff'], gy:['#888','#fff'] };
  const [bg,fg] = m[v]||m.dk;
  return { padding:'8px 16px', background:bg, color:fg, border:'none', borderRadius:6, fontWeight:500, cursor:'pointer', fontSize:13 };
};
const nbtn = active => ({ padding:'7px 14px', borderRadius:6, border:'1px solid #ccc', background: active?'#e8e8e8':'transparent', fontWeight: active?500:400, cursor:'pointer', fontSize:13, color:'#1a1a1a' });
const td = i => ({ padding:'6px 8px', borderBottom:'1px solid #eee', background: i%2===0?'#f9f9f9':'#fff' });
const badge = t => t==='out'
  ? { background:'#FADBD8', color:'#C0392B', borderRadius:20, padding:'2px 8px', fontSize:11, fontWeight:500, whiteSpace:'nowrap' }
  : t==='low'
  ? { background:'#FDEBD0', color:'#D35400', borderRadius:20, padding:'2px 8px', fontSize:11, fontWeight:500, whiteSpace:'nowrap' }
  : { background:'#D5F5E3', color:'#1E8449', borderRadius:20, padding:'2px 8px', fontSize:11, fontWeight:500, whiteSpace:'nowrap' };
const statLabel = t => t==='out'?'❌ หมดสต็อก':t==='low'?'⚠️ ใกล้หมด':'✅ ปกติ';
...
function Dashboard({ stock, sales }) {
  ...
  const stVal   = useMemo(() => PR.reduce((s,p) => {
    const d = stock[p[0]] || { qty:0, cost:p[4] };
    return s + d.qty * d.cost;
  }, 0), [stock]);
  ...
  const now2  = new Date();
  const todRev = sales.filter(s => new Date(s.date).toDateString()===now2.toDateString()).reduce((s,t)=>s+t.total,0);
  const monRev = sales.filter(s => { const d=new Date(s.date); return d.getMonth()===now2.getMonth()&&d.getFullYear()===now2.getFullYear(); }).reduce((s,t)=>s+t.total,0);
  const alerts = PR.filter(p => stat(stock[p[0]]?.qty||0, stock[p[0]]?.safety||10) !== 'ok').slice(0,20);

  const kpis = [
    ['📦 สินค้าทั้งหมด',   total+' รายการ'],
    ['💰 มูลค่าสต็อก',     '฿'+fmt(stVal)],
    ['💳 ยอดขายวันนี้',    '฿'+fmt(todRev)],
    ['📅 ยอดขายเดือนนี้',  '฿'+fmt(monRev)],
    ['❌ หมดสต็อก',         outN+' รายการ'],
    ['⚠️ ใกล้หมด',          lowN+' รายการ'],
  ];
  ...
  return (
    <div>
      ...
      {alerts.length > 0 && (
        <div style={css.card}>
          <div style={{fontWeight:500,marginBottom:10}}>⚠️ รายการที่ต้องดูแล ({alerts.length} รายการ)</div>
          <div style={css.tow}>
            <table style={css.tbl}>
              ...
              <tbody>
                {alerts.map((p,i) => {
                  ...
                  const t = stat(d.qty, d.safety);
                  return (
                    <tr key={p[0]}>
                      {[p[0],p[1],p[2],d.qty,d.safety].map((v,j)=><td key={j} style={td(i)}>{v}</td>)}
                      <td style={td(i)}><span style={badge(t)}>{statLabel(t)}</span></td>
                    </tr>
                  );
                })}
              </tbody>
...
function StockPage({ stock, setStock }) {
  ...
  const [stF, setStF]   = useState('ทั้งหมด');
  ...
  const startEdit = p => {
    ...
    setEdits(e => ({...e, [p[0]]:{qty:d.qty, safety:d.safety, cost:d.cost}}));
  };

  const saveAll = async () => {
    setSaving(true); setMsg('');
    const updates = Object.entries(edits).map(([sku,v]) => ({
      sku, qty:parseInt(v.qty)||0, safety:parseInt(v.safety)||10, cost:parseFloat(v.cost)||0
    }));
    const { error } = await supabase.from('stock').upsert(updates, {onConflict:'sku'});
    if (error) { setMsg('❌ บันทึกล้มเหลว: '+error.message); setSaving(false); return; }
    const newStock = {...stock};
    updates.forEach(u => { newStock[u.sku] = {qty:u.qty,safety:u.safety,cost:u.cost}; });
    setStock(newStock);
    setEdits({});
    setMsg('✅ บันทึก '+updates.length+' รายการเรียบร้อย!');
    setSaving(false);
  };
    ...
    <div>
      <div style={css.card}>
        <div style={{...css.row, gap:8}}>
          <input style={{...css.inp,flex:2,minWidth:180}} placeholder="ค้นหาชื่อ/SKU/ราคา (หลายคำคั่นช่องว่าง)" value={kw} onChange={e=>setKw(e.target.value)} />
          <select style={css.inp} value={cat} onChange={e=>setCat(e.target.value)}>
            {CATS.map(c=><option key={c}>{c}</option>)}
          </select>
          <select style={css.inp} value={stF} onChange={e=>setStF(e.target.value)}>
            {['ทั้งหมด','ปกติ','ใกล้หมด','หมดสต็อก'].map(s=><option key={s}>{s}</option>)}
          </select>
      ...
      {Object.keys(edits).length > 0 && (
        <div style={{...css.row, marginBottom:10, gap:8}}>
          <button style={btn('gr')} onClick={saveAll} disabled={saving}>
            {saving ? 'กำลังบันทึก...' : `💾 บันทึก ${Object.keys(edits).length} รายการ`}
          </button>
          <button style={btn('gy')} onClick={()=>setEdits({})}>ยกเลิก</button>
          {msg && <span style={{fontSize:12,color:msg.startsWith('✅')?'#1E8449':'#C0392B'}}>{msg}</span>}
        </div>
      )}
      <div style={css.tow}>
        <table style={css.tbl}>
          <thead><tr>
            {['#','SKU','ชื่อสินค้า','หมวด','ราคาขาย','ราคาทุน','สต็อก','Safety','มูลค่า','สถานะ','แก้ไข'].map((h,i)=><th key={i} style={css.th}>{h}</th>)}
          </tr></thead>
          <tbody>
            {filtered.map((p,i) => {
              const d  = stock[p[0]] || {qty:0,safety:10,cost:p[4]};
              ...
              return (
                <tr key={p[0]}>
                  ...
                  <td style={{...td(i),color:'#0000CC',fontWeight:500}}>{p[0]}</td>
                  <td style={td(i)}>{p[1]}</td>
                  <td style={td(i)}>{p[2]}</td>
                  <td style={td(i)}>฿{fmt(p[3])}</td>
                  <td style={td(i)}>฿{fmtD(ed?ed.cost:d.cost)}</td>
                  <td style={td(i)}>
                    {ed
                      ? <input type="number" min="0" style={{...css.inp,width:72,padding:'3px 6px',borderColor:'#0D47A1',background:'#E3F2FD',color:'#0D47A1',fontWeight:500,textAlign:'center'}} value={ed.qty} onChange={e=>setEdits(v=>({...v,[p[0]]:{...v[p[0]],qty:e.target.value}}))} />
                      : <b style={{color:'#0D47A1'}}>{d.qty}</b>}
                  </td>
                  <td style={td(i)}>
                    {ed
                      ? <input type="number" min="0" style={{...css.inp,width:58,padding:'3px 6px',borderColor:'#E65100',background:'#FFF3E0',color:'#E65100',textAlign:'center'}} value={ed.safety} onChange={e=>setEdits(v=>({...v,[p[0]]:{...v[p[0]],safety:e.target.value}}))} />
                      : d.safety}
                  </td>
                  <td style={td(i)}>฿{fmt(d.qty*d.cost)}</td>
                  <td style={td(i)}><span style={badge(t)}>{statLabel(t)}</span></td>
                  <td style={td(i)}>
                    {ed
                      ? <button style={{...btn('gy'),padding:'3px 10px',fontSize:11}} onClick={()=>setEdits(v=>{const n={...v};delete n[p[0]];return n;})}>✕</button>
                      : <button style={{...btn(),padding:'3px 10px',fontSize:11}} onClick={()=>startEdit(p)}>แก้ไข</button>}
                  </td>
                </tr>
              );
            })}
...
function SalesPage({ stock, setStock, setSales }) {
  ...
  const sugs = useMemo(() => {
    ...
    return PR.filter(p => {
      ...
      return words.every(w=>hay.includes(w)) && (stock[p[0]]?.qty||0)>0;
    }).slice(0,12);
  }, [kw, stock]);
  ...
  const addToCart = p => {
    setCart(c => {
      ...
      if (ex) return c.map(x=>x.sku===p[0]?{...x,qty:Math.min(x.qty+1,x.maxQty)}:x);
      return [...c, {sku:p[0],name:p[1],sell:p[3],cost:stock[p[0]]?.cost||p[4],qty:1,maxQty:stock[p[0]]?.qty||0}];
    });
    setKw('');
  };
  ...
  const confirmSale = async () => {
    ...
    setSaving(true); setMsg('');
    const items = cart.map(x=>({sku:x.sku,name:x.name,qty:x.qty,sell:x.sell,cost:x.cost,subtotal:x.sell*x.qty}));

    // Insert sale
    const { data: saleData, error: saleErr } = await supabase.from('sales').insert([{items,total,note}]).select();
    if (saleErr) { setMsg('❌ บันทึกล้มเหลว: '+saleErr.message); setSaving(false); return; }

    // Update stock
    const newStock = {...stock};
    const stockUpdates = [];
    for (const it of items) {
      const cur = newStock[it.sku]?.qty||0;
      const newQty = Math.max(0, cur-it.qty);
      newStock[it.sku] = {...(newStock[it.sku]||{}), qty:newQty};
      stockUpdates.push({sku:it.sku, qty:newQty, safety:newStock[it.sku]?.safety||10, cost:newStock[it.sku]?.cost||0});
    }
    const {error:stErr} = await supabase.from('stock').upsert(stockUpdates,{onConflict:'sku'});
    if (stErr) { setMsg('⚠️ บันทึกยอดขายแล้ว แต่ตัดสต็อกล้มเหลว'); setSaving(false); return; }

    setStock(newStock);
    setSales(s => [...s, saleData[0]]);
    setCart([]); setNote('');
    setMsg('✅ บันทึกการขาย ฿'+fmt(total)+' — ตัดสต็อกอัตโนมัติแล้ว');
    setSaving(false);
  };
    ...
    <div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) minmax(0,300px)',gap:12}}>
      <div>
        <div style={css.card}>
          ...
          <input style={{...css.inp,width:'100%'}} placeholder="พิมพ์ชื่อ/SKU — แสดงเฉพาะสินค้าที่มีสต็อก" value={kw} onChange={e=>setKw(e.target.value)} />
          {sugs.length > 0 && (
            <div style={{display:'flex',flexWrap:'wrap',gap:6,marginTop:8}}>
              {sugs.map(p=>(
                <button key={p[0]} style={{padding:'5px 10px',border:'1px solid #ddd',borderRadius:6,background:'#f5f5f5',cursor:'pointer',fontSize:12}} onClick={()=>addToCart(p)}>
                  <span style={{fontWeight:500}}>{p[1]}</span>
                  <span style={{color:'#1E8449',marginLeft:6}}>({stock[p[0]]?.qty||0})</span>
        ...
        {cart.length > 0 && (
          <div style={css.card}>
            ...
            <table style={css.tbl}>
              <thead><tr>{['ชื่อสินค้า','ราคา','จำนวน','รวม',''].map((h,i)=><th key={i} style={css.th}>{h}</th>)}</tr></thead>
              <tbody>
                {cart.map((x,i)=>(
                  <tr key={x.sku}>
                    <td style={td(i)}>{x.name}</td>
                    <td style={td(i)}>฿{fmt(x.sell)}</td>
                    <td style={td(i)}>
                      <input type="number" min="1" max={x.maxQty} style={{...css.inp,width:60,padding:'3px 6px'}} value={x.qty}
                        onChange={e=>setCart(c=>c.map(it=>it.sku===x.sku?{...it,qty:Math.max(1,Math.min(x.maxQty,parseInt(e.target.value)||1))}:it))} />
                    </td>
                    <td style={td(i)}>฿{fmt(x.sell*x.qty)}</td>
                    <td style={td(i)}><button style={{background:'none',border:'none',color:'#C0392B',cursor:'pointer',fontSize:18}} onClick={()=>setCart(c=>c.filter(it=>it.sku!==x.sku))}>×</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div>
        <div style={css.card}>
          ...
          <div style={{fontSize:30,fontWeight:500,color:'#1E8449',marginBottom:12}}>฿{fmt(total)}</div>
          <div style={{fontSize:12,color:'#666',marginBottom:4}}>หมายเหตุ (ช่องทาง, ลูกค้า ฯลฯ)</div>
          <input style={{...css.inp,width:'100%',marginBottom:12}} placeholder="เช่น Shopee, In-store..." value={note} onChange={e=>setNote(e.target.value)} />
          ...
          {msg && <div style={{marginTop:10,padding:'8px 10px',borderRadius:6,fontSize:12,background:msg.startsWith('✅')?'#D5F5E3':'#FADBD8',color:msg.startsWith('✅')?'#1E8449':'#C0392B'}}>{msg}</div>}
        </div>
      </div>
    </div>
  );
...
function ReportPage({ sales }) {
  ...
  const itemsCnt = filtered.reduce((s,t)=>s+t.items.reduce((a,b)=>a+b.qty,0),0);

  const skuMap={};
  filtered.forEach(t=>t.items.forEach(it=>{
    if(!skuMap[it.sku]) skuMap[it.sku]={name:it.name,qty:0,rev:0};
    skuMap[it.sku].qty+=it.qty; skuMap[it.sku].rev+=it.subtotal;
  }));
  const top=Object.entries(skuMap).sort((a,b)=>b[1].rev-a[1].rev).slice(0,10);

  return (
    <div>
      <div style={{...css.row,marginBottom:12}}>
        {[['today','วันนี้'],['week','7 วัน'],['month','เดือนนี้'],['all','ทั้งหมด']].map(([v,l])=>(
          <button key={v} style={nbtn(period===v)} onClick={()=>setPeriod(v)}>{l}</button>
        ))}
      </div>
      <div style={css.kgrid}>
        {[['ยอดขายรวม','฿'+fmt(revenue)],['จำนวนบิล',filtered.length+' บิล'],['ชิ้นที่ขาย',itemsCnt+' ชิ้น']].map(([l,v],i)=>(
          <div key={i} style={css.kcard}><div style={{fontSize:12,color:'#666',marginBottom:4}}>{l}</div><div style={{fontSize:20,fontWeight:500}}>{v}</div></div>
        ))}
      </div>
      {top.length>0&&(
        <div style={css.card}>
          <div style={{fontWeight:500,marginBottom:10}}>สินค้าขายดี Top 10</div>
          <div style={css.tow}><table style={css.tbl}>
            <thead><tr>{['#','SKU','ชื่อสินค้า','จำนวน','ยอดขาย'].map((h,i)=><th key={i} style={css.th}>{h}</th>)}</tr></thead>
            <tbody>{top.map(([sku,v],i)=>(
              <tr key={sku}>
                <td style={td(i)}>{i+1}</td>
                <td style={{...td(i),color:'#0000CC',fontWeight:500}}>{sku}</td>
                <td style={td(i)}>{v.name}</td>
                <td style={td(i)}>{v.qty}</td>
                <td style={td(i)}>฿{fmt(v.rev)}</td>
              </tr>
            ))}</tbody>
          </table></div>
        </div>
      ...
      <div style={css.card}>
        <div style={{fontWeight:500,marginBottom:10}}>ประวัติบิล</div>
        <div style={css.tow}><table style={css.tbl}>
          <thead><tr>{['วันที่','รายการ','ยอด','หมายเหตุ'].map((h,i)=><th key={i} style={css.th}>{h}</th>)}</tr></thead>
          <tbody>{filtered.slice().reverse().slice(0,50).map((s,i)=>(
            <tr key={s.id}>
              <td style={td(i)}>{thDT(s.date)}</td>
              <td style={td(i)}>{s.items.length} รายการ</td>
              <td style={{...td(i),fontWeight:500}}>฿{fmt(s.total)}</td>
              <td style={td(i)}>{s.note||'-'}</td>
            </tr>
          ))}</tbody>
        </table></div>
      </div>
...
function PlanPage({ stock, sales }) {
  ...
  }, [sales, stock, days, buffer]);

  return (
    <div>
      <div style={css.card}>
        <div style={{fontWeight:500,marginBottom:12}}>ตั้งค่าแผนการผลิต</div>
        <div style={{...css.row,gap:20}}>
          <div>
            <div style={{fontSize:12,color:'#666',marginBottom:4}}>คำนวณจากยอดขาย</div>
            <select style={css.inp} value={days} onChange={e=>setDays(Number(e.target.value))}>
              {[3,7,14,30].map(d=><option key={d} value={d}>{d} วัน</option>)}
            </select>
          </div>
          <div>
            <div style={{fontSize:12,color:'#666',marginBottom:4}}>Buffer เพิ่ม</div>
            <select style={css.inp} value={buffer} onChange={e=>setBuffer(Number(e.target.value))}>
              {[0,10,20,30,50].map(b=><option key={b} value={b}>{b}%</option>)}
            </select>
          </div>
          <div style={{fontSize:12,color:'#666',alignSelf:'flex-end',paddingBottom:6}}>
            แผนผลิตสำหรับ 14 วันข้างหน้า
          </div>
        </div>
      </div>
      {plan.length===0
        ? <div style={{...css.card,textAlign:'center',color:'#888'}}>ยังไม่มีข้อมูลยอดขายเพียงพอ — บันทึกการขายก่อนเพื่อให้ระบบคำนวณได้</div>
        : (
          <div style={css.card}>
            <div style={{fontWeight:500,marginBottom:10}}>แผนการผลิต ({plan.length} รายการ)</div>
            <div style={css.tow}><table style={css.tbl}>
              <thead><tr>{['#','SKU','ชื่อสินค้า','ขายใน '+days+'วัน','ขาย/วัน','สต็อก','ต้องผลิต ★'].map((h,i)=><th key={i} style={css.th}>{h}</th>)}</tr></thead>
              <tbody>{plan.map((r,i)=>(
                <tr key={r.sku}>
                  <td style={td(i)}>{i+1}</td>
                  <td style={{...td(i),color:'#0000CC',fontWeight:500}}>{r.sku}</td>
                  <td style={td(i)}>{r.name}</td>
                  <td style={td(i)}>{r.sold}</td>
                  <td style={td(i)}>{r.daily.toFixed(1)}</td>
                  <td style={td(i)}>{r.cur}</td>
                  <td style={td(i)}><b style={{color:'#C0392B'}}>{r.need}</b></td>
                </tr>
              ))}</tbody>
            </table></div>
          </div>
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      // Load stock
      const { data:stData, error:stErr } = await supabase.from('stock').select('*');
      if (stErr) { setError('ไม่สามารถเชื่อมต่อ Supabase ได้: '+stErr.message); setLoading(false); return; }
      const stockMap = {};
      stData.forEach(r=>{ stockMap[r.sku]={qty:r.qty,safety:r.safety,cost:r.cost}; });
      // Init missing SKUs
      const missing = PR.filter(p=>!stockMap[p[0]]).map(p=>({sku:p[0],qty:0,safety:10,cost:p[4]}));
      if (missing.length>0) {
        await supabase.from('stock').upsert(missing,{onConflict:'sku',ignoreDuplicates:true});
        missing.forEach(r=>{ stockMap[r.sku]={qty:r.qty,safety:r.safety,cost:r.cost}; });
      }
      setStock(stockMap);
      // Load sales
      const { data:salData } = await supabase.from('sales').select('*').order('date',{ascending:false}).limit(500);
      setSales(salData||[]);
      setLoading(false);
    })();
  }, []);
  ...
  const tabs = [
    ['dash','📊 Dashboard'],['stock','📦 สต็อก'],
    ['sales','💰 บันทึกขาย'],['report','📈 รายงาน'],['plan','🏭 แผนผลิต']
  ];

  if (loading) return <div style={{padding:40,textAlign:'center',color:'#888'}}>⏳ กำลังโหลดข้อมูล...</div>;
  if (error)   return <div style={{padding:40,textAlign:'center',color:'#C0392B'}}>❌ {error}<br/><small>ตรวจสอบ SUPABASE_URL และ SUPABASE_KEY ใน src/supabase.js</small></div>;

  return (
    <div style={css.wrap}>
      <div style={css.hdr}>
        <div>
          <div style={{fontWeight:500,fontSize:16}}>☕ สันติพาณิชย์ Stock System</div>
          <div style={{fontSize:11,color:'#7EC4A2',marginTop:2}}>ข้อมูล real-time — ทีมทุกคนเห็นพร้อมกัน</div>
        </div>
